...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Marcos/Ace\n' // criador
            + 'ORG: REM BOT;\n' // nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=5519988628741:+55 19 98862-8741\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.'
blocked = []
limitawal = '999999999'
...
async function starts() {
    ...
    •            @${num.split('@')[0]}was E j e c t e d
                      1 impostor restante   。　.
    　 　　。　　 　　　　ﾟ　　　.　      　　　.
    ,　　𝐀𝐯𝐢𝐬𝐨 𝐩𝐚𝐫𝐚 𝐯𝐨𝐜𝐞 𝐪𝐮𝐞 𝐬𝐚𝐢𝐮 @${num.split('@')[0]} Volte Sempre`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
      		...
      		mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD REM BOT*',
					benned: 'Você para a banido do bot, por favor, contate o proprietário para retirar seu ban',
					ownerG: '[❗] Ace? Este é um recurso especial para o Ace❌',
					ownerB: '[❗] Ace? Este é um recurso especial para o Ace ❌',
			...
			const ownerNumber = ["5519988628741@s.whatsapp.net"] // coloque seu numero
			const mod = [ownerNumber,"5519988628741@s.whatsapp.net"]//seu numero
			const adminbotnumber = ["5519988628741@s.whatsapp.net"]//seu numero
			const frendsowner = ["5519988628741@s.whatsapp.net"]//seu numero
			const premium = ["5519988628741@s.whatsapp.net","5519988628741@s.whatsapp.net",] 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("vai postar kwaii no grupo da sua familia seu mendigo")
		}, 0)
	}
	
	
	if (messagesC.includes("Nova Zelândia")){
			client.updatePresence(from, Presence.composing)
			reply("-41.282965,174.766473")
	}
	
		if (messagesC.includes("Zaragoza")){
			client.updatePresence(from, Presence.composing)
			reply("41.66167,-0.893427")
	}
	
		if (messagesC.includes("Pier 39")){
			client.updatePresence(from, Presence.composing)
			reply("37.8095,-122.4101")
	}
	
		if (messagesC.includes("Central Park")){
			client.updatePresence(from, Presence.composing)
			reply("40.780471,-73.963110")
	}
		if (messagesC.includes("Sidney")){
			client.updatePresence(from, Presence.composing)
			reply("-33.86498,151.21075")
	}
...
reply("oi posso ajudar")
	...
	if (messagesC.includes(".faustinho")){
			...
			switch(command) {
				...
				case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Marcos/Ace ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
...
case 'antilink':
				...
				//INFO BOT
case 'owner':
                    client.sendMessage(from, {displayname: "Marcos/Ace", vcard: vcard}, MessageType.contact, { quoted: mek})
...
break
          ...
          reply('Espere..')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5519988628741/*╰──────*「 * Marcos/Ace * 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO REM BOT_*\n*_Ketik #daftarvip*' })
					break
case 'daftarvip': 
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Marcos/Ace\n*YOUTUBE:* WPP:* wa.me/+5519988628741\nEspero que tenham gostado do bot'})
					break
				//_IMAGENS
				case 'boanoite':
					memein = await kagApi.memeindo()
